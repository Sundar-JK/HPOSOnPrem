...
import Com.API;
import Com.item_prices;
public class itemspriceget {
	public static void main(String[] args) {
        ...
        Gson gson = new Gson();
        item_prices[] item_pricesArray = gson.fromJson(info, item_prices[].class);
        System.out.println("done1");
        
    	Configuration cfg=new Configuration();
		cfg.configure("hibernate.cfg.xml");
		cfg.addAnnotatedClass(item_prices.class);
		...
		try {
			...
			for (item_prices spojo : item_pricesArray) {
	               
				item_prices entity = new item_prices();
