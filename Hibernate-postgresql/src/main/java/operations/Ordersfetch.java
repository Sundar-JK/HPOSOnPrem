   	  ...
   	  String newTableQuery = "FROM temptrnordhdrss WHERE transaction_no = :Id";
		  ...
		  List<Headers> HeadersInfoList = new ArrayList();
	   		...
	   		for(temptrnordhdrss data:temptrnordhdrList) {
	   			int detailid=data.getId();
	   			System.out.println(detailid);
		   		System.out.println("drewwww");
		   		
		   		String detailquerry="FROM temptrnorddet WHERE parent_order_ref_no = :detailid";
		   	  List<temptrnorddet> Tempdetail = session.createQuery(detailquerry)
	                    .setParameter("detailid", String.valueOf(detailid), StandardBasicTypes.STRING)
	                    .list();
	   			...
	   			int headerrrid=data.getId();
	   			
	   			
	   			
	   			String detaildquery="FROM temptrnorddet WHERE parent_order_ref_no = :headerrrid";
	   		    List<temptrnorddet> Tempdetail1 = session.createQuery(detaildquery)
	                    .setParameter("headerrrid", String.valueOf(headerrrid), StandardBasicTypes.STRING)
	                    .list();
	            ...
	            for (temptrnorddet detaildata : Tempdetail) {
	   			...
	   			headerinfo.setDetails(DetailsInfoList);
...
//	   	 
//	        for (temptrnordhdrss data : temptrnordhdrList) {
//	            int transactionno = TempTabletrn.getId();
