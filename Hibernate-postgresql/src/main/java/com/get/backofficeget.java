...
import pojo.backofficetabless;

public class backofficeget {

	public static void main(String[] args) {
	    String preurl = "http://d365hposvmv1.uaenorth.cloudapp.azure.com:4003/HPOSServerUAT/public/api/";        
        String fullUrl = preurl + API.backofficetables ;
        String info = executeGet(fullUrl);
        System.out.println(info);
        
        Gson gson = new Gson();
        backofficetabless[] backofficetablessArray = gson.fromJson(info, backofficetabless[].class);
        System.out.println("done1");
        
    	Configuration cfg=new Configuration();
		cfg.configure("hibernate.cfg.xml");
		cfg.addAnnotatedClass(backofficetabless.class);
		...
		try {
			...
			for (backofficetabless spojo :backofficetablessArray) {
	               
				backofficetabless entity = new backofficetabless();
                System.out.println("done5");
                entity.setId(spojo.getId());
                entity.setCreated_at(spojo.getCreated_at());
                entity.setUpdated_at(spojo.getUpdated_at());
                entity.setTable_name(spojo.getTable_name());
