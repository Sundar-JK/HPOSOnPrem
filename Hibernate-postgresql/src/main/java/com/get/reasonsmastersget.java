...
public class reasonsmastersget {

	
	public void fetchreasonmasters() {
	    String preurl = "http://d365hposvmv1.uaenorth.cloudapp.azure.com:4003/HPOSServerUAT/public/api/";        
        String fullUrl = preurl + API.reason_masters ;
        String info = executeGet(fullUrl);
        System.out.println(info);
        
        Gson gson = new Gson();
        reasonmasters[] reasonmastersArray = gson.fromJson(info, reasonmasters[].class);
		...
		try {
			...
			for (reasonmasters spojo :reasonmastersArray) {
	               
				reasonmasters entity = new reasonmasters();
                System.out.println("done5");
                entity.setId(spojo.getId());
                entity.setCreated_at(spojo.getCreated_at());
                entity.setUpdated_at(spojo.getUpdated_at());
                entity.setReason(spojo.getReason());
                entity.setReason_type(spojo.getReason_type());
