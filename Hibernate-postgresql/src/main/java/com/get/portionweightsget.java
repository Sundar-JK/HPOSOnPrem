...
public class portionweightsget {

	public void fetchportionweights() {
	    String preurl = "http://d365hposvmv1.uaenorth.cloudapp.azure.com:4003/HPOSServerUAT/public/api/";        
        String fullUrl = preurl + API.portion_weights ;
        String info = executeGet(fullUrl);
        System.out.println(info);
        
        Gson gson = new Gson();
        portionweights[] portionweightsArray = gson.fromJson(info, portionweights[].class);
		...
		try {
			...
			for (portionweights spojo :portionweightsArray) {
	               
				portionweights entity = new portionweights();
                System.out.println("done5");
                entity.setId(spojo.getId());
                entity.setCreated_at(spojo.getCreated_at());
                entity.setUpdated_at(spojo.getUpdated_at());
                entity.setPortion_weight_label(spojo.getPortion_weight_label());
                entity.setPortion_weight_code(spojo.getPortion_weight_label());
