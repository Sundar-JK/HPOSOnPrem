...
public class costingmethodsgets {
	public void fetchcostings() {
	    String preurl = "http://d365hposvmv1.uaenorth.cloudapp.azure.com:4003/HPOSServerUAT/public/api/";        
        String fullUrl = preurl + API.costing_methods ;
        String info = executeGet(fullUrl);
        System.out.println(info);
        
        Gson gson = new Gson();
        costingmethods[] costingmethodsArray = gson.fromJson(info, costingmethods[].class);
		...
		try {
			...
			for (costingmethods spojo :costingmethodsArray) {
	               
				costingmethods entity = new costingmethods();
                System.out.println("done5");
                entity.setId(spojo.getId());
                entity.setCreated_at(spojo.getCreated_at());
                entity.setUpdated_at(spojo.getUpdated_at());
                entity.setCosting_label(spojo.getCosting_label());
                entity.setCosting_code(spojo.getCosting_code());
